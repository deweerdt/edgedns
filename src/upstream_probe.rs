...
fn compute_probe_name(probe_suffix: &[u8]) -> Result<Vec<u8>, &'static str> {
    ...
    let h = hasher.finish();
    probe_key.write_u64::<NativeEndian>(h).unwrap();
...
fn main() {
    let name = compute_probe_name(b"fastly").unwrap();
    println!("{:?}", String::from_utf8(name.clone()).unwrap());
    verify_probe_name(&name, b"fastly").unwrap();
    //verify_probe_name(b"\x1eedgedns-probe-D9z0WCWIVcDaO-na\x00", b"").unwrap();
}
